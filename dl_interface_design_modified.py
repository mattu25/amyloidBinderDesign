...
parser.add_argument( "-outputScores", type=str, default=None ) #Added for outputting scores
...
class ProteinMPNN_runner():
    ...
    def __init__(self, args, struct_manager):
        ...
        self.scoreFile = args.outputScores #Added for outputting scores
    ...
    def sequence_optimize(self, sample_feats):
        ...
        sequences, times = mpnn_util.generate_sequences(
            self.mpnn_model,
            self.device,
            feature_dict,
            arg_dict,
    ...
    def proteinmpnn(self, sample_feats):
        ...
        '''
        seqs_scores, times = self.sequence_optimize(sample_feats)
        #print(f"\n\n Scores: {seqs_scores}\n\n----------------")
        #print(f"\n\nType: {type(seqs_scores)}")
        ...
        if args.outputScores != None:
            csv_filename = args.outputScores
            with open(csv_filename, mode='a', newline='') as csvfile:
                csvwriter = csv.writer(csvfile)

                if csvfile.tell() == 0:
                    csvwriter.writerow(['sequence_number', 'sequence', 'score', "time_from_start"])  

                for index, (sequence, score) in enumerate(seqs_scores):
                    csvwriter.writerow([index, sequence, score, times[index]])
